...
def retrieve_from_vectorstore(
    ...
    # DIAGNOSTIC CODE: Check what collection name is generated and if it has points
    from backend.vector_store import get_collection_name, qdrant_client
    coll_name = get_collection_name(session_id)
    
    try:
        info = qdrant_client.get_collection(collection_name=coll_name)
        print(f"\n[DEBUG] Agent is searching collection: {coll_name}")
        print(f"[DEBUG] Total documents/points in this collection: {info.points_count}\n")
    except Exception as e:
        print(f"\n[DEBUG] Collection {coll_name} does not exist yet: {e}\n")
